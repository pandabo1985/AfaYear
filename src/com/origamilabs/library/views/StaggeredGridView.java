...
public class StaggeredGridView extends ViewGroup {
	...
	public void onRestoreInstanceState(final Parcelable state) {
		...
		final ArrayList<ColMap> convert = ss.mapping;

		if (convert != null) {
			mColMappings.clear();
			for (final ColMap colMap : convert) {
				mColMappings.add(colMap.values);
			}
		}

		if (ss.firstId >= 0) {
			mFirstAdapterId = ss.firstId;
	...
	public Parcelable onSaveInstanceState() {
		...
		if (getChildCount() > 0) {
			...
			final int topOffsets[] = new int[mColCount];
			...
			if (mColWidth > 0) {
				for (int i = 0; i < mColCount; i++) {
					if (getChildAt(i) != null) {
						...
						while (left > col * (mColWidth + mItemMargin * 2) + getPaddingLeft()) {
							col++;
						}
			...
			final ArrayList<ColMap> convert = new ArrayList<ColMap>();
			for (final ArrayList<Integer> cols : mColMappings) {
	...
	@Override
	protected boolean checkLayoutParams(final ViewGroup.LayoutParams lp) {
	...
	protected int[] onCreateDrawableState(final int extraSpace) {
		...
		if (mIsChildViewEnabled) // Common case
	...
	private final boolean contentFits() {
		if (mFirstPosition != 0 || getChildCount() != mItemCount) return false;
	...
	private View getFirstChildAtColumn(final int column) {
	...
	private void populate(final boolean clearData) {
	...
	private void positionSelector(final int l, final int t, final int r, final int b) {
	 ...
	 *         prematurely.
	 */
	private boolean trackMotionScroll(final int deltaY, final boolean allowOverScroll) {
	...
	private void useDefaultSelector() {
		setSelector(getDefaultSelector(getContext()));
	}
	 ...
	 */
	ContextMenuInfo createContextMenuInfo(final View view, final int position, final long id) {
	...
	final int fillDown(final int fromPosition, final int overhang) {
		...
		while (nextCol >= 0 && mItemBottoms[nextCol] < fillTo && position < mItemCount) {
			...
			if (!mColMappings.get(nextCol).contains(position)) {
				...
				for (final ArrayList<Integer> list : mColMappings) {
	...
	final int fillUp(final int fromPosition, final int overhang) {
		...
		while (nextCol >= 0 && mItemTops[nextCol] > fillTo && position >= 0) {
			...
			final int itemTop = mItemTops[nextCol];
	 ...
	 */
	final int getNextColumnDown(final int position) {
		int result = -1;
		int topMost = Integer.MAX_VALUE;
	...
	final LayoutRecord getNextRecordDown(final int position, final int span) {
	...
	final void invalidateLayoutRecordsAfterPosition(final int position) {
	...
	final void invalidateLayoutRecordsBeforePosition(final int position) {
		int endAt = 0;
	...
	final void layoutChildren(final boolean queryAdapter) {
		...
		for (int i = 0; i < childCount; i++) {
			...
			if (queryAdapter) {
				...
				final View newView = obtainView(position, child);
				if (newView == null) {
	...
	final void offsetChildren(final int offset) {
		final int childCount = getChildCount();
		for (int i = 0; i < childCount; i++) {
			final View child = getChildAt(i);
	...
	void positionSelector(final int position, final View sel) {
	...
	private static Drawable getDefaultSelector(final Context context) {
		final TypedArray a = context.obtainStyledAttributes(null, new int[] { android.R.attr.listSelector },
				android.R.attr.gridViewStyle, android.R.style.Widget_GridView);
	...
	public static class LayoutParams extends ViewGroup.LayoutParams {
		...
		public LayoutParams(final Context c, final AttributeSet attrs) {
			...
			if (width != MATCH_PARENT) {
				Log.w(TAG, "Inflation setting LayoutParams width to " + width + " - must be MATCH_PARENT");
				width = MATCH_PARENT;
			}
		...
		public LayoutParams(final ViewGroup.LayoutParams other) {
			...
			if (width != MATCH_PARENT) {
				Log.w(TAG, "Constructing LayoutParams with width " + width + " - must be MATCH_PARENT");
				width = MATCH_PARENT;
			}
			if (height == MATCH_PARENT) {
	...
	private static final class LayoutRecord {
		...
		public final int getMarginAbove(final int col) {
			if (mMargins == null) return 0;
			return mMargins[col * 2];
		}

		public final int getMarginBelow(final int col) {
			if (mMargins == null) return 0;
			return mMargins[col * 2 + 1];
		}

		public final void setMarginAbove(final int col, final int margin) {
			if (mMargins == null && margin == 0) return;
			ensureMargins();
			mMargins[col * 2] = margin;
		}

		public final void setMarginBelow(final int col, final int margin) {
			if (mMargins == null && margin == 0) return;
			ensureMargins();
	...
	private class RecycleBin {
		...
		public void addScrap(final View v) {
		...
		public View getScrapView(final int type) {
		...
		public View getTransientStateView(final int position) {
			if (mTransientStateViews == null) return null;
		...
		public void setViewTypeCount(final int viewTypeCount) {
			if (viewTypeCount < 1)
				throw new IllegalArgumentException("Must have at least one view type (" + viewTypeCount
						+ " types reported)");
			if (viewTypeCount == mViewTypeCount) return;
	...
	static class ColMap implements Parcelable {
		...
		public static final Creator<ColMap> CREATOR = new Creator<ColMap>() {
			@Override
			public ColMap createFromParcel(final Parcel source) {
				return new ColMap(source);
			}

			@Override
			public ColMap[] newArray(final int size) {
				return new ColMap[size];
			}
		};

		public ColMap(final ArrayList<Integer> values) {
			this.values = values;
		}

		private ColMap(final Parcel in) {
			tempMap = in.createIntArray();
			values = new ArrayList<Integer>();
			for (final int element : tempMap) {
				values.add(element);
			}
		}
		...
		int[] toIntArray(final ArrayList<Integer> list) {
